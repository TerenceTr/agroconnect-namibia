// ============================================================================
// frontend/src/pages/dashboards/farmer/FarmerQualityAnalyticsPage.jsx
// ----------------------------------------------------------------------------
// FILE ROLE:
//   Farmer-facing complaint analytics dashboard for Phase 4B + Phase 4C.
//
// PHASE 4C ADDITIONS:
//   ✅ Repeat issue risk panel
//   ✅ Alert threshold visibility
//   ✅ Top repeat issue cards and recommendations
// ============================================================================
...
import { fetchFarmerReviewAnalytics } from "../../../services/reviewAnalyticsApi";
import { fetchFarmerRepeatIssueDetection } from "../../../services/repeatIssueApi";
...
export default function FarmerQualityAnalyticsPage() {
  const { user } = useAuth();
    ...
    async function load() {
      ...
      try {
        const [analyticsData, repeatData] = await Promise.all([
          fetchFarmerReviewAnalytics(farmerId, filters),
          fetchFarmerRepeatIssueDetection(farmerId, filters),
        ]);
        if (!active) return;
  ...
  const productBreakdown = safeArray(payload?.product_breakdown);
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-3 rounded-3xl border border-[#D8F3DC] bg-white p-6 shadow-sm lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-900">
              Complaint trends, issue hotspots, and repeat signals
            </h1>
            <p className="mt-2 max-w-3xl text-sm font-medium leading-6 text-slate-600">
              Track which complaint categories affect your products most, where recurring issues are emerging,
              and how resolution performance is changing over time.
            </p>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <KpiCard
            ...
            subtitle="Structured issue tags captured"
            icon={AlertTriangle}
            tone="amber"
          />
          <KpiCard
            ...
            subtitle="Issues still needing action"
            icon={ShieldCheck}
            tone="rose"
          />
          <KpiCard
            title="Average rating"
            value={safeNumber(summary?.avg_rating, 0).toFixed(2)}
            subtitle="Across tagged complaint reviews"
            icon={TrendingUp}
            tone="emerald"
          />
          <KpiCard
            ...
            subtitle="Recurring issue hotspots detected"
            icon={Filter}
          />
        </div>
        ...
        <div className="rounded-3xl border border-[#D8F3DC] bg-white p-5 shadow-sm">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <label className="text-sm font-semibold text-slate-700">
              Parent group
              <select
                value={filters.parent_group}
                onChange={(e) => setFilter("parent_group", e.target.value)}
                className="mt-1 w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm"
              >
                <option value="">All groups</option>
                {parentGroups.map((group) => (
                  <option key={group} value={group}>
                    {safeStr(group).replace(/_/g, " ")}
                  </option>
                ))}
              </select>
            </label>
            ...
            <label className="text-sm font-semibold text-slate-700">
              Taxonomy item
              <select
                value={filters.taxonomy_code}
                onChange={(e) => setFilter("taxonomy_code", e.target.value)}
                className="mt-1 w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm"
              >
                <option value="">All complaint types</option>
                {taxonomyItems.map((item) => (
                  <option key={item.taxonomy_id || item.code} value={item.code}>
                    {item.label}
                  </option>
                ))}
              </select>
            ...
            <label className="text-sm font-semibold text-slate-700">
              ...
              <select
                ...
                onChange={(e) => setFilter("resolution_status", e.target.value)}
                className="mt-1 w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm"
              >
            ...
            <label className="text-sm font-semibold text-slate-700">
              Detected by
              <select
                value={filters.detected_by}
                onChange={(e) => setFilter("detected_by", e.target.value)}
                className="mt-1 w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm"
              >
            ...
            <label className="text-sm font-semibold text-slate-700">
              ...
              <select
                ...
                onChange={(e) => setFilter("min_severity", Number(e.target.value))}
                className="mt-1 w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm"
              >
            ...
            <label className="text-sm font-semibold text-slate-700">
              Repeat threshold
              <select
                value={filters.repeat_threshold}
                onChange={(e) => setFilter("repeat_threshold", Number(e.target.value))}
                className="mt-1 w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm"
              >
          ...
          <div className="mt-4 flex flex-wrap gap-4">
            ...
            <label className="inline-flex items-center gap-2 text-sm font-semibold text-slate-700">
              <input
                type="checkbox"
                checked={filters.only_negative}
                onChange={(e) => setFilter("only_negative", e.target.checked)}
              />
        ...
        {loading ? (
          ...
          <>
            <RepeatIssueRiskPanel payload={riskPayload} title="Repeat issue risk panel" />
            ...
            <div className="grid gap-4 xl:grid-cols-2">
              <div className="rounded-3xl border border-[#D8F3DC] bg-white p-5 shadow-sm">
                <div className="mb-4 text-sm font-extrabold text-slate-900">Product complaint hotspots</div>
                  ...
                  <table className="min-w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-200 text-left text-slate-500">
                        ...
                        <th className="px-3 py-2 font-semibold">Avg rating</th>
                      </tr>
                    </thead>
                    <tbody>
                      {productBreakdown.slice(0, 10).map((row) => (
                        <tr key={row.product_id} className="border-b border-slate-100">
                          <td className="px-3 py-2 font-semibold text-slate-900">{row.product_name}</td>
                          ...
                          <td className="px-3 py-2">{safeNumber(row.avg_rating).toFixed(2)}</td>
                        </tr>
                      ))}
              ...
              <div className="rounded-3xl border border-[#D8F3DC] bg-white p-5 shadow-sm">
                ...
                <div className="space-y-3">
                  {repeatClusters.slice(0, 10).map((cluster, index) => (
